...
if (!$currentUser) {
    header('Location: /login?redirect=/new-article');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!isset($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'] ?? '', $_POST['csrf_token'])) {
        ...
        if (!$title || !$content) {
            ...
            try {
                createArticle($title, $content, $currentUser['id']);
...
<head>
    ...
    <title>Write a Story - MultiBlog</title>
...
<body>
    ...
    <main class="page-container">
        <div class="form-container" style="max-width: 800px;">
            <h1 class="form-title">Write a new story</h1>
            ...
            <form method="POST" action="/new-article">
                ...
                <div class="form-group">
                    ...
                    <textarea id="content" name="content" required rows="15"
                              class="form-input"
                              placeholder="Share your story..."><?php echo htmlspecialchars($_POST['content'] ?? ''); ?></textarea>
                </div>

                <div style="display: flex; gap: 1rem; margin-top: 2rem;">
                    <button type="submit" class="btn btn-primary" style="flex: 1;">Publish</button>
                    <a href="/my-articles" class="btn btn-outline">Cancel</a>
                </div>
            </form>
        </div>
    </main>
